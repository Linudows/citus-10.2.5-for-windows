...
{
#ifndef PG_NO_DEBUG	
	fprintf(stdout, "[PG_DEBUG]enter StartNodeUserDatabaseConnection(%s,%d,%s,%s)\n", hostname, port, user, database);
#endif	
...
{
#ifndef PG_NO_DEBUG	
	fprintf(stdout, "[PG_DEBUG]enter StartConnectionEstablishment()\n");
#endif	
	...
	// 										  false);
	connection->pgConn = PQconnectdbParams((const char **) entry->keywords,
											  (const char **) entry->values,
											  false);
